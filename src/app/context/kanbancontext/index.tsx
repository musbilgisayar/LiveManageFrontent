...
import { deleteFetcher, getWebFetcher, postWebFetcher } from "@/utils/fetchers.web.client";
...
export const KanbanDataContextProvider: React.FC<KanbanDataContextProps> = ({ children }) => {
    ...
    const { data: todosData, isLoading: isTodosLoading, error: todoError, mutate } = useSWR("/api/kanban", getWebFetcher,)
    useEffect(() => {
        if (todosData) {
    ...
    const addCategory = async (categoryName: string) => {
        try {
            const response = await mutate(postWebFetcher('/api/kanban/add-category', { categoryName }), false);
