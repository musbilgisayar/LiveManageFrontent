...
import { deleteFetcher, getWebFetcher, postWebFetcher, putFetcher } from "@/utils/fetchers.web.client";
...
export const ContactContextProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const { data: contactsData, isLoading: isContactsLoading, error: contactsError, mutate } = useSWR('/api/contacts', getWebFetcher);
  ...
  const addContact = async (newContact: ContactType) => {
    try {
      await mutate(postWebFetcher("/api/contacts", newContact));
    } catch (error) {
