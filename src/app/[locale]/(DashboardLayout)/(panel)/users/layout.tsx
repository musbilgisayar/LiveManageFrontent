// File: src/app/[locale]/(DashboardLayout)/(panel)/superadmin/users/layout.tsx
...
async function fetchModuleDict(locale: string): Promise<Dict> {
  ...
  if (!json) {
    console.error("[UsersLayout] Çeviri preload yanıtı parse edilemedi", {
      locale,
      namespaces: MODULE_NS,
      responseStatus: response.status,
    });
  ...
  if (!json.ok) {
    console.warn("[UsersLayout] Çeviri preload kısmi hatayla geldi", {
      locale,
      namespaces: MODULE_NS,
      failed: json.failed,
      error: json.error,
...
export default async function UsersLayout({ children, params }: Props) {
  ...
  try {
    ...
    if (Object.keys(dict).length === 0) {
      console.warn(
        "[UsersLayout] Çeviri sözlüğü boş geldi, alt ağaç doğrudan render ediliyor.",
        {
          lang,
          namespaces: MODULE_NS,
        }
